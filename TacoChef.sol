// Special thanks to Polyshield.finance!
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    uint256 public constant MAX_EMISSION_RATE = 1000000000000000000; // recommended by Paladin to set a max rate
    // Deposit Fee address
    ...
    function pendingTaco(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0 && totalAllocPoint > 0) {  // fix for paladin low finding
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (nftaddr!=address(0x0)){
            ...
            tacoparty.mint(nftaddr, tacoReward.div(20));
            tacoparty.mint(burnaddr, tacoReward.div(20));
        }else{
            tacoparty.mint(burnaddr, tacoReward.div(10));
        }
        tacoparty.mint(address(this), tacoReward);
    ...
    function harvest(uint256 _pid) public nonReentrant {
        ...
        uint256 pending = 0;
        if (user.amount > 0) {
             pending = user.amount.mul(pool.accTacoPerShare).div(1e12).sub(user.rewardDebt);
            if (pending > 0) {
                safeTacoTransfer(msg.sender, pending);
            }
        }
    ...
    function safeTacoTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > tacoBal) {
            transferSuccess = tacoparty.transfer(_to, tacoBal);
        } else {
            transferSuccess = tacoparty.transfer(_to, _amount);
        }
        require(transferSuccess, "safeTacoTransfer: transfer failed");
    }

    function setFeeAddress(address _feeAddress) public onlyOwner {
        require(_feeAddress != address(0), "not nonzero"); // fix applied for Paladin High risk
        feeAddress = _feeAddress;
   ...
   function updateEmissionIfNeeded() public {
    ...
    function getTacoPriceCents() public view returns (uint256 spc){
         uint tacoBalance = tacoparty.balanceOf(usdcTacoLP);
          if (tacoBalance > 0) {
            uint256 priceCents = usdc.balanceOf(usdcTacoLP).mul(1e14).div(tacoBalance);
            return priceCents;
          }
          return 0;
    }
    ...
    function getEmissionRatePercent(uint256 tacoPriceCents) public view returns (uint256 epr) {
       ...
       uint256 tacoPricePercentOfTop = tacoPriceCents.mul(100).div(topPrice*100);
       int256 curveCoefficient = FixidityLib.reciprocal(int256(curveRate));
       uint256 f1 = uint256(100).sub(tacoPricePercentOfTop);
       int256 p1 = FixidityLib.fromFixed(FixidityLib.multiply(curveCoefficient,int256(f1)));
       int256 p2 = FixidityLib.fromFixed(FixidityLib.divide(100,int256(tacoPricePercentOfTop)));
    ...
    //Only update before start of farm  // fix for low severity finding from Paladin
